...
*/
?>
<?php define("PLUGINURL", dirname(__FILE__) );
...
class ideal_interactive_map{
	...
	var $default_selected_map_color = "#fff";
	...
	function shortcode( $atts, $content = null){
		...
		$close = plugins_url( 'img/mapclose.png' , __FILE__ );
                $drop = plugins_url( 'img/mapdrop.png' , __FILE__ );
                $dropdown = plugins_url( 'img/mapdropdown.png' , __FILE__ );
    	...
    	<div id="map-container">
            <div id="mapdiv" data-lat="{$zoomLatitude}" data-long="{$zoomLongitude}" data-zoom="{$zoomLevel}" style="width: 100%;  height: 500px;"></div>
            <div id="desc_overlay" class="animated">
                    <h2 class="pop_desc_title"></h2><span id="ico_drop"><img class="mapdrop" src="$drop" align="middle" /><img class="mapdropdown" src="$dropdown" align="middle" /></span><span id="ico_close"><img src="$close" align="middle" /></span>
                    <div class="flag"></div>
                    <div id="overlay-content"></div>
                    <a id="readmore">More info</a> 
            </div>
                    <div class="errorbox">Only 3 countries are allowed <span class="errorboxxlose"></span></div>
                    <div class="errorboxalreadyclicked">You cant select again already selected country <span class="errorboxxlosealreadyclicked"></span></div>
            </div>
              <div class="infoboxescontainer"></div>  
    	</div>
    	<div id="mapreadmore">
    	</div> 
                    </div></div>
                     <div class="worldmapbottompart"> 
            <div class="container">        
                <div class="row clearfix"> 
                    <div class="column col-md-8" style="float:none;margin:auto;padding:0;">
                        <div class="bottomtextmap">
Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostru </div>
                    </div>
                </div>  
            </div>
        </div>
          <div class="dbinfowrap">
        <div class="container">        
            <div class="row clearfix">
                <div class="dbinfotext">Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostruLorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat. Ut wisi enim ad minim veniam, quis nostru</div>
            </div>
        </div>
    </div>
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) { $the_query->the_post();
				...
				$map_color = get_post_meta($id, "map_color", true);
                                $excerpt = get_post_meta($id, "shorttext", true);
				if($country_code && isset($countries[$country_code])){
						
					$areas[] = array(
										"id"=>$country_code,
										"groupId"=>$country_code,
                                                                                "excerpt" => ((!$this->disabled_flag) ?
                                                                                        '<div class="phoca-flagbox"><span class="phoca-flag '. strtolower($country_code) .'"></span></div>' : '') .$excerpt,
                                                                                "custom"=> array(
	...
	echo <<<xxxx
		<script type="text/javascript">
		    ...
		    jQuery(document).ready(function ($) {
			 ...
			 jQuery('.maptitledrop').each(function(){
                            jQuery(this).toggle(
                             function() {
                               jQuery(this).parent().parent().find('.mapcontent').fadeOut();
                               jQuery(this).parent().parent().find('#readmore').fadeOut();
                             }, function() {
                               jQuery(this).parent().parent().find('.mapcontent').fadeIn();
                               jQuery(this).parent().parent().find('#readmore').fadeIn();
                             }
                          );
                        }); 
		     function onregionClick(country){
				...
				jQuery("#overlay-content").html(obj.custom.desc);
                                var finalContent = '<div class="mapbox"><div class="maptitle"><div class="maptitlemain">'+obj.title+'</div><div class="flag"></div></div><div class="maptitlerightpart"><div class="maptitledrop"></div><div class="maptitleclose"></div></div><div class="mapcontent">'+obj.excerpt+'</div><a id="readmore">More info</a><div class="fullinfo"><div class="maptitlemain">'+obj.title+'</div>'+obj.custom.desc+'</div></div>';
                                var count = jQuery(".infoboxescontainer .mapbox").length;
                               
                                if(count < 3){
                                    jQuery('.errorbox').fadeOut(); 
                                   var onetitle = jQuery('.pop_desc_title').text();
                                   jQuery('.infoboxescontainer').prepend(finalContent);
                                    var onetitle = jQuery('.pop_desc_title').text(); 
                                    ...
                                    /*jQuery('.infoboxescontainer .mapbox').each(function(){
                                        alltitles =  jQuery(this).find('.maptitle .maptitlemain').text();
                                        
                                        if(jQuery(this).find('.maptitle .maptitlemain:contains(onetitle)')){
                                            jQuery(this).find('.maptitle .maptitlemain').css("text-decoration", "underline");
                                        }
                                        if(onetitle == alltitles) { 
                                            //alert('You already selected this country') // matching title;  
                                            myObject.push({
                                              'title' : alltitles
                                            })
                                        }
                                    ...
                                    console.log(myObject);*/
                                     ...
                                     jQuery('.errorbox').fadeIn();
                                     jQuery('.errorboxxlose').click(function(){
                                       jQuery('.errorbox').fadeOut();    
                                     });
                                }
                                jQuery('.infoboxescontainer .mapbox .mapcontent').each(function(){
                                  var flaghidden = jQuery(this).find('.phoca-flagbox'); 
                                  var flag = jQuery(this).parent().find('.maptitle .flag').append(flaghidden);
                                });
                                jQuery('.maptitleclose').each(function(){
                                ...
                                jQuery('a#readmore').each(function(){
                                    jQuery(this).click(function(){
                                        jQuery('.moreinfowrap').html('');
                                        jQuery('.moreinfowrap .fullinfo').text();
                                        var readmoretext = jQuery(this).parent().find('.fullinfo').html();
                                        jQuery('.moreinfowrap').html(readmoretext);
                                        jQuery('.moreinfowrapper').fadeIn();
                                         jQuery('html, body').animate({
                                               scrollTop: jQuery(".moreinfowrapper").offset().top - 100
                                         }, 'slow');
                                   });
                                });
                                jQuery("#ico_drop").toggle(
                                    function() {
                                     jQuery('#overlay-content').fadeOut();
                                     jQuery('#readmore').fadeOut();
                                     jQuery('.mapdrop').fadeOut(10);
                                     jQuery('.mapdropdown').fadeIn(10);
                                    }, function() {
                                     jQuery('#overlay-content').fadeIn();
                                     jQuery('#readmore').fadeIn();
                                     jQuery('.mapdropdown').fadeOut(10);
                                     jQuery('.mapdrop').fadeIn(10);
                                ...
                                jQuery('.maptitledrop').each(function(){
                                    jQuery(this).toggle(
                                     function() {
                                jQuery(this).parent().parent().animate({ height: '274px','margin-top':"-216px"}, 1000);
                                       jQuery(this).parent().parent().find('.mapcontent').fadeIn();
                                       jQuery(this).css({"background":"url('http://imcnew.ipoint.com.mt/wp-content/plugins/ideal-interactive-map/img/mapdrop.png')"});
                                       jQuery(this).parent().parent().find('#readmore').fadeIn();
                                     }, function() {
                                     jQuery(this).parent().parent().animate({ height: '58px','margin-top':"0px"}, 1000);
                                       jQuery(this).parent().parent().find('.mapcontent').fadeOut();
                                        jQuery(this).css({"background":"url('http://imcnew.ipoint.com.mt/wp-content/plugins/ideal-interactive-map/img/mapdropdown.png')"});
                                       jQuery(this).parent().parent().find('#readmore').fadeOut();
                                     }
                                  );
                                }); 
			        ...
			        return el.css('cursor', opt.cursor).on("mousedown", function(e) {
			            ...
			            if(opt.handle === "") {
			                ...
			                jQuery(this).removeClass('active-handle').parent().removeClass('draggable');
                              ...
                              jQuery('.infoboxescontainer .mapbox .maptitledrop').click(function(){
                                    alert('aaa');
                                }); 
			})(jQuery);
        </script>
...
xxxx;
	...
	function mappoints($code = ""){
			...
			$countries = array(
                    ...
                    "CI"=> array("CГґte d'Ivoire", 7.539989, -5.54708),
                    ...
                    "RE"=> array("RГ©union", -21.115141, 55.536384),
